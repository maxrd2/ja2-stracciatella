...
void BltVideoSurface(SGPVSurface* const dst, SGPVSurface* const src, INT32 const iDestX, INT32 const iDestY, SGPBox const* const src_box)
{
	...
	if(src_bpp == dst_bpp) {
		SDL_Rect src_rect;
		if(src_box) {
			src_rect.x = src_box->x;
			src_rect.y = src_box->y;
			src_rect.w = src_box->w;
			src_rect.h = src_box->h;
		}

		SDL_Rect dstrect = { iDestX, iDestY, 0, 0 };
		SDL_BlitSurface(src->surface_, src_box ? &src_rect : NULL, dst->surface_, &dstrect);
	} else {
