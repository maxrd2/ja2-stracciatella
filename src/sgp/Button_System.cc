...
GUIButtonRef CreateTextButton(const wchar_t *string, SGPFont const font, UINT32 sForeColor, UINT32 sShadowColor, INT16 xloc, INT16 yloc, INT16 w, INT16 h, INT16 Priority, GUI_CALLBACK ClickCallback)
{
	// if button size is too small, adjust it.
	if (w < 4) w = 4;
	if (h < 3) h = 3;
...
GUIButtonRef CreateIconAndTextButton(BUTTON_PICS* const Image, const wchar_t* const string, SGPFont const font, const UINT32 sForeColor, const UINT32 sShadowColor, const UINT32 sForeColorDown, const UINT32 sShadowColorDown, const INT16 xloc, const INT16 yloc, const INT16 Priority, const GUI_CALLBACK ClickCallback)
{
...
GUIButtonRef CreateLabel(const wchar_t* text, SGPFont const font, UINT32 forecolor, UINT32 shadowcolor, INT16 x, INT16 y, INT16 w, INT16 h, INT16 priority)
{
...
void GUI_BUTTON::SpecifyGeneralTextAttributes(wchar_t const* const string, SGPFont const font, UINT32 const fore_color, UINT32 const shadow_color)
{
	SpecifyText(string);
	usFont        = font;
